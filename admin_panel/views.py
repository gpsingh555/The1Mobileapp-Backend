        ...
        if User.objects.filter(email=email).exists():
            ...
            if user is not None:
                if user.is_staff == True:
                    ...
                    return HttpResponse(json.dumps(returnMessage),content_type = 'application/javascript; charset=utf8',status=HTTP_200_OK)
                else:
                    return Response({"status": "fail",'message':'you are not authorized'},status=HTTP_400_BAD_REQUEST)
            else:
                return Response({"status": "fail",'message':'Email and password is incorrect'},status=HTTP_400_BAD_REQUEST)
        else:
...
class view_users(APIView):
    ...
    def get(self,request,*args,**kwargs):
        context={}
        if request.user.is_staff == True or SubAdminPermission.objects.filter(user=request.user,user_mgmt=True).exists():
            context={}
...
class verify_forget_otp(APIView):
	def post(self,request):
		...
		if check_blank_or_null([email,otp]) and User.objects.filter(email=email).exists() and User.objects.filter(email=email).exists():
			user=User.objects.get(email=email)
...
class CreateSubAdminApiView(APIView):
    ...
    def post(self, request):
        context={}
        if request.user.is_staff == True or SubAdminPermission.objects.filter(user=request.user,subadmin_mgmt=True).exists():
            ...
            if serializer.is_valid():
                ...
                return Response({'data': serializer.data,'message': 'Sub Admin Created Successfully','Token':token.key}, status=HTTP_200_OK)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        else:
            context['message']="You are not authorized for this operation"
            context['status']="fail"
    ...
    def post(self, request,*args,**kwargs):
        ...
        if request.user.is_staff == True or SubAdminPermission.objects.filter(user=request.user,subadmin_mgmt=True).exists():
            ...
            if check_blank_or_null([user_id]) and User.objects.filter(pk=user_id).exists():
                ...
                obj.save()
                return Response({'message':'Subadmin And Subadmin Permissions Updated Successfully'},status=HTTP_200_OK)
            else:
