		...
		if check_blank_or_null([mobile_number]) and User.objects.filter(username=mobile_number).exists():
			...
			if profileO.isotp_verified == False:
				if signup_otp.objects.filter(user=user,expire__gte=datetime.now(),otp=otp).exists():
					sotp=signup_otp.objects.get(user=user,expire__gte=datetime.now(),otp=otp)
					if sotp.attempt < 5:
	...
	def post(self,request):
		...
		if check_blank_or_null([mobile_number]) and User.objects.filter(username=mobile_number).exists():
			...
			if profileO.isotp_verified == False:
				...
				otp=123456
				fo=signup_otp.objects.create(user=user,expire=datetime.now()+timedelta(minutes=1440),otp=otp)
...
class UserAddressListApiView(ListCreateAPIView):
    ...
    def get(self,request, *args, **kwargs):
        ...
        if pk:
            ...
            print(qs,'qs.................')
            if qs.exists():
                data = UserCountrySerializer(qs, context={'request': request}, many=True).data
        else:
            queryset = savecountry.objects.filter(user=request.user).order_by('-id')
            if queryset.exists():
                data = UserCountrySerializer(queryset, context={'request': request}, many=True).data

        return Response({'data':data}, status=HTTP_200_OK)

    def post(self, request, *args, **kwargs):
        data=request.data
        serializer = UserCountryCreateSerializer(data=data, context={'request':request})
...
class UserRequestNews(APIView):
    permission_classes = (IsAuthenticated,)
    def post(self,request):
        serializer=NewsRequestCreateSerializer(data=request.data,context={'request':request})
...
class BitcoinNews(APIView):
  def post(self,request):
    ...
    if check_blank_or_null([keyword]) and News.objects.filter(keyword=keyword).exists():
      ...
      newsO=News.objects.filter(keyword=keyword,block=False).order_by("-id")[:30] 
      data={
