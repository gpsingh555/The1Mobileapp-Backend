...
def upload_news():
    for keyword in list(news_keyword.objects.all().values_list("keyword",flat=True)):
        ...
        for x in all_articles['articles']: 
            ...
            print(x['description'])
            print(x['url'])
            print(datetime.strptime(x['publishedAt'].split("T")[0]+" "+x['publishedAt'].split("T")[-1].replace("Z",""), "%Y-%m-%d %H:%M:%S"))
            ...
            if News.objects.filter(title=x['title']).exists() == False:
                ...
                ews.publishedAt=datetime.strptime(x['publishedAt'].split("T")[0]+" "+x['publishedAt'].split("T")[-1].replace("Z",""), "%Y-%m-%d %H:%M:%S")
